...
export class CreateinvoiceComponent {
  constructor(
    ...
    this.invoiceForm = this.builder.group({
      rows: this.builder.array([]) // Initialize the FormArray
    });
    this.rowsFormArray = this.invoiceForm.get('rows') as FormArray;
  }
  ...
  getSellers() {
    this.api.getAllSellers().subscribe(response => this.sellerList = response)
  }

  getClients() {
    this.api.getAllClients().subscribe(response => this.clientList = response)
  }

  onSellerChange(selectedsellerid: any) {
    const selectedSeller = this.sellerList.find(seller => seller.id === parseInt(selectedsellerid.target.value));
    this.seller = selectedSeller;
  }

  onClientChange(selectedclientid: any) {
    const selectedClient = this.clientList.find(client => client.id === parseInt(selectedclientid.target.value));
    this.client = selectedClient;
  }

  addRow() {
    this.rowsFormArray.push(this.createRowFormGroup());
  }
